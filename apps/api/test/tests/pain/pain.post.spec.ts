...
describe(`POST /${painMeta.endpoint}`, () => {
  ...
  it(`POST /${painMeta.endpoint} → 403 when unauthenticated`, async () => {
    ...
    await request(app.getHttpServer()).post(`/${painMeta.endpoint}`).send(newPain).expect(403);
  });

  it(`POST /${painMeta.endpoint} → 201 when authenticated user creates a pain`, async () => {
  ...
  it(`POST /${painMeta.endpoint} → 400 when the body is malformed`, async () => {
    const newPain = {
      data: {
        type: painMeta.endpoint,
        id: "",
  ...
  it(`POST /${painMeta.endpoint} → 400 when required fields are missing`, async () => {
    const newPain = {
      data: {
        type: painMeta.endpoint,
  ...
  it(`POST /${painMeta.endpoint} → 400 when relationship ID is invalid UUID`, async () => {
    const newPain = {
      data: {
        type: painMeta.endpoint,
  ...
  it(`POST /${painMeta.endpoint} → 400 when relationship ID does not exist`, async () => {
    const newPain = {
      data: {
        type: painMeta.endpoint,
  ...
  it(`POST /${painMeta.endpoint} → 400 when relationship type is incorrect`, async () => {
  ...
  it(`POST /${painMeta.endpoint} → 201 when creating with populated relationship arrays`, async () => {
    // Use company-filtered fixtures for relationship IDs
    const companyClassificationFixtures = Object.values(CLASSIFICATIONS).filter(r => r.company?.id === COMPANIES.CompanyOne.id);
