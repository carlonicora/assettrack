...
export class S3Service {
  ...
  private async _loadConfiguration(): Promise<void> {
    const type = this.config.get<ConfigS3Interface>("s3").type;
    ...
    switch (type) {
      case "s3":
        ...
        this.s3Client = new S3Client({
          region: this.config.get<ConfigS3Interface>("s3").region,
          credentials: {
            accessKeyId: this.config.get<ConfigS3Interface>("s3").key,
            secretAccessKey: this.config.get<ConfigS3Interface>("s3").secret,
          },
        });
        break;
      case "digitalocean":
      case "hetzner":
        this._bucket = this.config.get<ConfigS3Interface>("s3").bucket;
        this._endpoint = `https://${this._bucket}.${this.config.get<ConfigS3Interface>("s3").endpoint.split("//")[1]}/`;
        this.s3Client = new S3Client({
          region: this.config.get<ConfigS3Interface>("s3").region,
          endpoint: this.config.get<ConfigS3Interface>("s3").endpoint,
          credentials: {
            accessKeyId: this.config.get<ConfigS3Interface>("s3").key,
            secretAccessKey: this.config.get<ConfigS3Interface>("s3").secret,
          },
      ...
      case "minio":
        this._bucket = this.config.get<ConfigS3Interface>("s3").bucket;
        this._endpoint = this.config.get<ConfigS3Interface>("s3").endpoint;
        this.s3Client = new S3Client({
          endpoint: this.config.get<ConfigS3Interface>("s3").endpoint,
          region: "local",
          credentials: {
