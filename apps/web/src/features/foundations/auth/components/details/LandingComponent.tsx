  ...
  return (
    <>
      ...
      <CardFooter className="mt-4 flex w-full flex-col justify-between">
        ...
        {process.env.NEXT_PUBLIC_USE_GOOGLE === "true" && (
          <Button
            variant="outline"
            className="mt-4 flex w-full items-center"
            onClick={(e) => {
              e.preventDefault();
              window.location.href = `${process.env.NEXT_PUBLIC_API_URL}googleusers/auth`;
            }}
          >
            ...
            {t(`foundations.auth.buttons.continue_with_google`)}
          </Button>
        )}
      </CardFooter>
    </>
